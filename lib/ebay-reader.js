...
export function createEbayReader({ credentials, fetchImpl = fetch, now = Date.now }) {
  ...
  return {
    async reputation() {
      ...
      if (identity.User?.UserID?.toLowerCase() !== 'gearhubparts') throw new Error('Unexpected seller.');
      const feedback = await readCall('GetFeedback', '<UserID>gearhubparts</UserID><FeedbackType>FeedbackReceivedAsSeller</FeedbackType>');
      return { positive: identity.User.PositiveFeedbackPercent, summary: feedback.FeedbackSummary,
        feedbackCount: feedback.FeedbackDetailItemTotal, feedbackScore: feedback.FeedbackScore };
    },
    ...
    async itemDetails(id) {
      ...
      const result = await readCall('GetItem', `<ItemID>${id}</ItemID><DetailLevel>ReturnAll</DetailLevel><IncludeItemSpecifics>true</IncludeItemSpecifics><IncludeItemCompatibilityList>true</IncludeItemCompatibilityList>`);
